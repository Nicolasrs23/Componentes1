  ...
  const disabled=""
    ...
    const submitHandler = (event) => {
        ...
        if (saveName.length>1 && saveMail === saveMail2 && savePhone.length>1)  {
            ...
            alert('validacion fallida Corroborar que todos los datos son correctos');
            event.preventDefault();
        }
    ...
    return (
        <form onSubmit={submitHandler}>
            ...
            <label><input type="email" placeholder="re ingrese su email" onChange={(event) => setSaveMail2(event.target.value)}/></label>
            <label><input type="number" placeholder="cellphone number" onChange={(event) => setSavePhone(event.target.value)}></input></label>
            <Button id="btnBuy" type="submit" disabled={disabled}>Submit</Button>
        </form>
    )
}
